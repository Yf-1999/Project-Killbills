...
"""Nous n'avons plus le problème de non-convergence
...
model_arbres = DecisionTreeClassifier()
model_arbres.fit(X_train_res, y_train_res)

# Évaluation du modèle
y_pred = model_arbres.predict(X_test_vectors)
print(classification_report(y_test, y_pred))
...
"""# modèle fixé et la fonction de classification
Pour conclure, apres le meilleur modèle que nous avons trouvé est le modèle d'arbres de décision. Nous fixons alors ce modèle et créons une fonction qui prendra en entrée un "item" et renverra la catégorie associée à cet "item".
"""

# Fonction de classification
def classify_item_arbres(item):
    item_vector = vectorizer.transform([item])
    category = model_arbres.predict(item_vector)
    return category[0]